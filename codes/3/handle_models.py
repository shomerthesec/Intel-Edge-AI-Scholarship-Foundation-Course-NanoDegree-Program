...
def handle_pose(output, input_shape):
    ...
    # heatmaps for heat points and points for the full lines
    # heatmaps = output['Mconv7_stage2_L2']
    points = output['Mconv7_stage2_L1']
    # TODO 2: Resize the heatmap back to the size of the input
    # Create an empty array to handle the output map
    out_heatmap = np.zeros([points.shape[1], input_shape[0], input_shape[1]])

    # Iterate through and re-size each heatmap
    for h in range(len(points[0])):
        out_heatmap[h] = cv2.resize(points[0][h], input_shape[0:2][::-1])
